...
import encyclopediaRaw from '../../Assets/Widgets/Barra_Menu_Superiore/tasto-enciclopedia.svg?raw';
...
export function stripForInline(svg: string): string {
  return svg.replace(/ id="[^"]*"/g, '').replace(/<title>.*?<\/title>\s*/, '');
}

export const gearIcon = stripForInline(gearRaw);
export const encyclopediaIcon = stripForInline(encyclopediaRaw);
